...
class Multiset
{
	...
	private:
		struct DataCounterNode
		{
			DataCounterNode(ItemType val):value(val),count(1),next(nullptr),prev(nullptr){}
			DataCounterNode(ItemType val, int num):value(val),count(num),next(nullptr),prev(nullptr){}
			ItemType value;
			int count;
			DataCounterNode* next;
			DataCounterNode* prev;
		};
		DataCounterNode* head;
		DataCounterNode* tail;
