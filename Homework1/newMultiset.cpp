...
Multiset::Multiset():m_numItems(0), m_numUniqueItems(0), m_maxSize(DEFAULT_MAX_ITEMS)
{
	m_itemArr=new DataCounter[m_maxSize];
	for (int i=0;i<m_maxSize;i++)
		m_itemArr[i].count=0;
}
Multiset::Multiset(int size):m_numItems(0), m_numUniqueItems(0), m_maxSize(size)
{
	m_itemArr=new DataCounter[m_maxSize];
	for (int i=0;i<m_maxSize;i++)
		m_itemArr[i].count=0;
}
Multiset::Multiset(const Multiset& src):m_numItems(src.m_numItems), m_numUniqueItems(src.m_numUniqueItems), m_maxSize(src.m_maxSize)
{
	m_itemArr=new DataCounter[m_maxSize];
	for (int i=0;i<m_maxSize;i++)
		m_itemArr[i]=src.m_itemArr[i];
}
Multiset::~Multiset()
{
	delete[] m_itemArr;
